...
DTrain = DBins{1};
LTrain = LBins{1};
XTest  = combineBins(XBins, 2:size(XBins,2));
DTest  = combineBins(DBins, 2:size(DBins,2));
LTest  = combineBins(LBins, 2:size(LBins,2));
...
numIterations = 3500;  % Change this, number of iterations (epochs)
learningRate  = 0.03;  % Change this, your learning rate
W0 = randn(size(XTrain, 2), numHidden)/size(XTrain, 1); % Initialize your weight matrix W
V0 = randn(size(DTrain, 2), numHidden+1)/size(DTrain, 1); % Initialize your weight matrix V
